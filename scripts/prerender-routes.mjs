...
for (const { path: rp, component } of routes) {
  ...
  prerenderedPaths.push(rp);
  console.log(`  ✅ ${rp} → ${meta.title.substring(0, 55)}`);
  success++;
}
// Generate _redirects: explicit prerendered routes first, then SPA fallback
const redirects = [
  '# Auto-generated by scripts/prerender-routes.mjs',
  '# Prerendered routes - serve their own index.html (force flag = ! to override SPA fallback)',
  ...prerenderedPaths.map(p => `${p.padEnd(45)} ${p}/index.html  200!`),
  '',
...
console.log(`\n📄 Generated dist/_redirects with ${prerenderedPaths.length} explicit routes`);
