...
const renderMessageWithLinks = (text: string) => {
  ...
  while ((match = markdownLinkRegex.exec(text)) !== null) {
    ...
    parts.push(<a key={`link-${match.index}`} href={match[2]} target="_blank" rel="noopener noreferrer" className="text-primary hover:text-primary/80 underline">
        {match[1]}
      </a>);
...
const Chatbot = () => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      try {
        ...
        responseData = {
          message: "Antwort erhalten, aber konnte nicht verarbeitet werden."
        };
      }
  ...
  return <>
      ...
      {/* Chat Window */}
      {isOpen && <div className="fixed bottom-1 sm:bottom-24 left-1 right-1 sm:right-6 sm:left-auto sm:w-96 h-[40vh] sm:h-[500px] max-h-[300px] sm:max-h-[500px] bg-card border border-card-border rounded-2xl shadow-elevated z-50 flex flex-col animate-scale-in" style={{
      height: 'clamp(250px, 40vh, 300px)'
          ...
          <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3 sm:space-y-4 bg-muted/20">
            {messages.map(message => <div key={message.id} className={`flex ${message.isUser ? 'justify-end' : 'justify-start'} animate-fade-in`}>
                <div className={`max-w-[85%] sm:max-w-[80%] p-2 sm:p-3 rounded-xl sm:rounded-2xl text-xs sm:text-sm ${message.isUser ? 'bg-gradient-primary text-primary-foreground' : 'bg-card border border-card-border text-foreground'}`}>
                  <div className="flex items-start space-x-1 sm:space-x-2">
                    {!message.isUser && <Bot className="w-3 h-3 sm:w-4 sm:h-4 mt-0.5 text-accent flex-shrink-0" />}
                    {message.isUser && <User className="w-3 h-3 sm:w-4 sm:h-4 mt-0.5 text-primary-foreground flex-shrink-0" />}
                     <div>
          ...
          <div className="p-3 sm:p-4 border-t border-card-border bg-card rounded-b-2xl">
            <div className="flex space-x-1 sm:space-x-2">
              <input type="text" value={inputMessage} onChange={e => setInputMessage(e.target.value)} onKeyPress={handleKeyPress} placeholder="Ihre Frage zu KI-Lösungen..." className="flex-1 px-2 sm:px-3 py-2 border border-input rounded-lg sm:rounded-xl bg-background text-foreground text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-ring transition-smooth" disabled={isLoading} />
              <Button onClick={sendMessage} disabled={!inputMessage.trim() || isLoading} className="bg-gradient-primary text-primary-foreground hover-scale" size="sm">
